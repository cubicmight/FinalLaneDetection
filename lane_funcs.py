...
def warp_perspective(frame):
    ...
    matrix = cv2.getPerspectiveTransform(source_points,
                                         destination_points)  # Matrix to warp the image for skyview window
    ...
    if lines is not None:  # If there no lines we take line in memory
        ...
        for line in lines:  # Access each line in lines scope
            if len(memory_x1_list) < 10 and len(memory_x2_list) < 10:  # Including memory feature
                ...
                if abs(x1 - avg_x1) > width * 0.21 and abs(
                        x2 - avg_x2) > width * 0.21:  # Here we check the difference between new lines
        ...
        if len(left_fit) > 0:  # Here we ckeck whether fit for the left line is valid
            ...
            lane_lines.append(
                map_coordinates(frame, left_fit_average))  # Add result of mapped points to the list lane_lines
        else:
        ...
        if len(right_fit) > 0:  # Here we ckeck whether fit for the right line is valid
            ...
            lane_lines.append(
                map_coordinates(frame, right_fit_average))  # Add result of mapped points to the list lane_lines
        else:
...
def map_coordinates(frame, parameters):
    ...
    try:
        ...
        slope, intercept = 0, 0  # Unpack slope and intercept from the given parameters
...
def process_frame(frame):
    ...
    if len(fps_list) == 60:  # Calculate avg fps every timestamp
        ...
        fps_list = []
    cv2.putText(final_frame, "AVG FPS: " + str(avg_fps), (50, 150), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 0, 0), 2,
                cv2.LINE_AA)  # Draw AVG FPS

    return final_frame  # Return final frame


def arrow_detection_left(frame, template_image):
    global threshold
    # Read the main image
    img_rgb_left = cv2.imread(frame)
    # Convert it to grayscale
    img_gray = cv2.cvtColor(img_rgb_left, cv2.COLOR_BGR2GRAY)
    # Read the template
    ...
    for pt in zip(*loc[::-1]):
        cv2.rectangle(img_rgb_left, pt, (pt[0] + w, pt[1] + h), (0, 255, 0), 2)
...
def arrow_detection_right(frame, template_image):
    global threshold
    # Read the main image
    img_rgb_right = cv2.imread(frame)
    # Convert it to grayscale
    img_gray = cv2.cvtColor(img_rgb_right, cv2.COLOR_BGR2GRAY)
    # Read the template
    template = cv2.imread(template_image, 0)
    # Store width and height of template in w and h
    w, h = template.shape[::-1]
